...
import { getMovieListsByUserId, addTVShowToList, getMovieListById } from "../../../api/movieStorage";
...
export default function TVCard({ tv }) {
        ...
        const fetchData = async () => {
            try {
                if (user) {
                    ...
                    setTVLists(fetchedTVLists.filter(list => {
    ...
    React.useEffect(() => {
        if (tv) {
            const localURL = tv.poster_path ? `https://image.tmdb.org/t/p/w500/${tv.poster_path}` : tv.backdrop_path ? `https://image.tmdb.org/t/p/w500/${tv.backdrop_path}` : defaultImage;
    ...
    const handleChange = async (event) => {
        ...
        try {
            ...
            if (tvList.tvs.find(t => t.id === tv.id)) throw new Error('TV show already in list');
